...
void worker(std::stop_token stop_token) {
    ...
    grids[0] = std::make_unique<ParticleGrid>(width, height);
    grids[1] = std::make_unique<ParticleGrid>(width, height);
...
void to_qimage(QImage &image) {
    ...
    image = QImage((uint8_t *)buf.data(), width, height, QImage::Format_ARGB32);
}
...
void update(ParticleGrid *grid) {
    ...
    for (size_t y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            buf[y * width + x] = grid->get_cell(x, y).rgb.to_pixel();
        }
    }
    frame_idx++;
}
